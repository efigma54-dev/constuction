...
export default function StickyInquiryRail() {
  ...
  const waNumber = company.whatsapp?.replace(/\D/g, "") ?? "919876543210";
  const waLink = `https://wa.me/${waNumber}?text=${encodeURIComponent("Hi, I'm interested in Aakar Heights. Can I schedule a site visit?")}`;

  return (
    <div className={`sticky-inquiry-rail${visible ? " is-visible" : ""}`} aria-hidden={!visible}>
      {/* WhatsApp */}
      <a
        href={waLink}
        target="_blank"
        rel="noopener noreferrer"
        className="sticky-rail-btn sticky-rail-btn--whatsapp"
